...
=head2 $retries = $bal->init([$flag])

Get/set the init flag
...
sub add_route {
	...
	$self->firewall_rule(FORWARD => undef,
			     ...
			     -o    => $device,
			     -d    => $network,
			     -j    => 'ACCEPT') for ($self->lan_services, $self->isp_sub_services);
	$self->firewall_rule(FORWARD => undef,
			     -i    => $device,
			     -s    => $network,
			     -o    => $self->dev($_),
...
=head2 @names = $bal->isp_sub_services

Return list of service names that correspond to load-balanced ISPs - with subrole of LAN.

=cut

sub isp_sub_services {
    my $self = shift;
    return grep {$self->_sub($_)} $self->isp_services;
}

=head2 @names = $bal->isp_nat_services

Return list of service names that correspond to load-balanced ISPs - NAT.

=cut

sub isp_nat_services {
    my $self = shift;
    return grep {$self->nat($_)} $self->isp_services;
}
...
=head2 @names = $bal->isp_up_services

Return the number of ISPs up.

=cut

sub isp_up_services {
    my $self = shift;
    return grep {$self->up($_)} $self->isp_services;
}
...
sub nat	{ shift->_service_field(shift,'nat')  }
sub _sub { shift->_service_field(shift,'_sub')  }
...
sub _parse_configuration_file {
	...
	my ($service,$device,$role,$ping_dest,$weight,$nat,$gw,$sub_role) = split /\s+/;
	...
	$services{$service}{nat}   = $nat        || 'yes';
	$services{$service}{gw}    = $gw;
	$services{$service}{_sub}  = $sub_role;
    }
    close $f;
...
#     print STDERR Dumper($i);
	...
	$interface_info->{$svc} = {
	    ...
	    net     => $info->{net},
	    ip      => $info->{ip},
	    fwmark  => $role eq 'isp' ? ++$counter * 10 : undef,
	    table   => $role eq 'isp' ?   $counter : undef,
	    role    => $role,
	    ping    => $s->{$svc}{ping},
	    weight  => $s->{$svc}{weight},
	    nat	    => $s->{$svc}{nat} eq 'yes' ? 1 : 0,	
	    _sub    => $s->{$svc}{_sub},	
	}
    }
...
sub _create_default_multipath_route {
	...
	$self->ip_route("replace default scope global $hops");
    } 

    else {
	print STDERR "# setting single default route via $up[0]n" if $self->verbose;
	$self->ip_route("replace default via",$self->gw($up[0]),'dev',$self->dev($up[0]));
    }
...
sub _create_default_failover_route {
    ...
    $self->ip_route("replace default via",$self->gw($preferred),'dev',$self->dev($preferred));
}
...
#		     '-A INPUT -d 127.0.0.0/8 -j DROPPERM']);
   ...
   for my $lan ($self->lan_services, $self->isp_sub_services) {
	my $dev = $self->dev($lan);
	my $net = $self->net($lan);
...
sub _lan_lan_forwarding_rules {
    ...
    my @lans = ($self->lan_services, $self->isp_sub_services);
    for (my $i=0;$i<@lans;$i++) {
	for (my $j=0;$j<@lans;$j++) {
	    next if $i == $j;
	    my $lan1 = $lans[$i];
